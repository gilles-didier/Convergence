...
static int getSizeLexiTreeRec(TypeLexiTree *dict, int n);
static void fillLexiTreeRec(char **tab, char *tmp, int i, int n, TypeLexiTree *dict);

void sprintDoubleLatex(char *s, int p, double d) {
	double n;
	char tmp[50];
	n = floor(log(d)/log(10));
	if(n == 0.) {
		sprintf(tmp, "$%%.%dlf$", p);
		sprintf(s, tmp, d);
	} else {
		sprintf(tmp, "$%%.%dlf\\!\\times\\!10^{%%.0lf}$", p);
		sprintf(s, tmp, d/pow(10., n), n);
	}
}
...
TypeLexiTree *getDictNameTab(char **name, int size) {
    ...
    for(i=0; i<size; i++)
        if(name && name[i]) {
            if(addWordLexi(name[i], i, dict) != END_INT)
...
void fprintLexiTreeRec(FILE *f, char *tmp, int i, int n, TypeLexiTree *dict) {
    int c;
    for(c=dict->node[n].child; c != END_INT; c=dict->node[c].sibling) {
...
int getSizeLexiTreeRec(TypeLexiTree *dict, int n) {
    int c;
...
int getSizeLexiTree(TypeLexiTree *dict) {
		int c, res = 0;
		for(c=dict->node[dict->root].child; c != END_INT; c=dict->node[c].sibling)
			res += getSizeLexiTreeRec(dict, c);
	return res;
}
...
/*int findWordLexi(char *w, TypeLexiTree *dict) {
    ...
    while(i<MAX_DICT_LENGTH && w[i] != '\0') {
        int c;
        c = dict->node[n].child;
        while(c != END_INT && dict->node[c].symbol<w[i])
            c = dict->node[c].sibling;
        if(c != END_INT && dict->node[c].symbol == w[i]) {
            n = c;
            i++;
        } else
            return END_INT;
    }
    if(dict->node[n].child  != END_INT && dict->node[dict->node[n].child].symbol == '\0')
